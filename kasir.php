...
<body class="bg-brand-offwhite h-screen flex overflow-hidden text-brand-black">
    ...
    <aside
        class="w-24 bg-brand-black shadow-xl flex-hidden flex-col items-center py-6 md:flex border-r border-white/10 z-20">
        <div
            class="w-12 h-12 bg-brand-red rounded-xl flex items-center justify-center text-white font-bold text-xl mb-8 shadow-[0_4px_14px_rgba(230,57,70,0.4)]">
            <i class="fa-solid fa-mug-hot"></i>
        </div>
        <div class="flex flex-col gap-4 w-full px-4">
            <button
                class="w-full aspect-square bg-brand-red/20 text-brand-red rounded-2xl flex flex-col items-center justify-center gap-1 transition-transform hover:scale-105 border border-brand-red/30">
                <i class="fa-solid fa-border-all text-xl"></i>
                <span class="text-[10px] font-bold uppercase tracking-wider mt-1">Semua</span>
            </button>
            <button
                class="w-full aspect-square text-gray-400 hover:bg-white/10 hover:text-white rounded-2xl flex flex-col items-center justify-center gap-1 transition-transform hover:scale-105">
        ...
        <a href="logout.php"
            class="mt-auto w-12 h-12 bg-white/5 text-gray-400 rounded-xl flex items-center justify-center hover:bg-brand-red hover:text-white transition-colors">
    ...
    <main class="flex-1 flex flex-col bg-brand-offwhite p-6 overflow-y-auto relative">
        <header class="flex justify-between items-center mb-6">
            ...
            <div class="relative">
                <input type="text" placeholder="Cari menu..."
                    class="pl-10 pr-4 py-3 bg-white border border-black/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-red shadow-sm w-64">
        ...
        <?php if (isset($success_msg)): ?>
            <div
                class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6 rounded-lg font-semibold shadow-sm">
        ...
        <?php if (isset($error_msg)): ?>
            <div
                class="bg-red-100 border-l-4 border-brand-red text-brand-darkred p-4 mb-6 rounded-lg font-semibold shadow-sm">
                <?php echo $error_msg; ?>
            </div>
        <?php endif; ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <?php if (empty($menus)): ?>
                <div
                    class="col-span-full p-10 text-center text-brand-gray border-2 border-dashed border-black/10 rounded-2xl">
            ...
            <?php foreach ($menus as $m): ?>
                <form method="POST" action="">
                    ...
                    <button type="submit"
                        class="w-full text-left bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden hover:-translate-y-1 hover:shadow-xl hover:border-brand-red/30 transition-all group relative">
                        <div class="h-40 overflow-hidden relative bg-gray-100">
                            <img src="<?php echo htmlspecialchars($m['foto_produk']); ?>"
                                class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                onerror="this.src='https://via.placeholder.com/300'">
                            <div
                                class="absolute top-3 right-3 bg-brand-black/80 backdrop-blur text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                                <?php echo htmlspecialchars($m['kategori']); ?>
                            </div>
                        </div>
                        <div class="p-5">
                            <h3 class="font-extrabold text-brand-black text-lg leading-tight mb-1">
                                <?php echo htmlspecialchars($m['nama_produk']); ?>
                            </h3>
                            <p class="text-brand-red font-black text-xl">Rp
                                <?php echo number_format($m['harga'], 0, ',', '.'); ?>
                            </p>
                        </div>
                        <div
                            class="absolute inset-0 bg-brand-red/0 group-hover:bg-brand-red/5 transition-colors pointer-events-none">
                        </div>
                    </button>
                </form>
            <?php endforeach; ?>
    ...
    <aside class="w-96 bg-white shadow-2xl flex flex-col z-30 border-l border-black/5">
            ...
            <div class="flex items-center gap-3">
                <div class="text-right">
                    ...
                    <p class="text-[10px] text-white/50 uppercase tracking-widest">Shift
                        <?php echo htmlspecialchars($kasir->getShift()); ?>
                    </p>
                </div>
                <!-- Menggunakan foto dari database -->
                <img src="<?php echo htmlspecialchars($kasir->getFotoProfile() ?: 'https://i.pravatar.cc/150?img=5'); ?>"
                    class="w-10 h-10 rounded-full border border-white/20 object-cover"
                    onerror="this.src='https://i.pravatar.cc/150?img=5'">
            </div>
        </div>
                ...
                <?php foreach ($cart as $idx => $item): ?>
                    <div class="flex flex-col bg-white border border-black/5 p-4 rounded-2xl shadow-sm">
                        <div class="flex justify-between items-start mb-2">
                            <h4 class="font-bold text-brand-black leading-tight flex-1 pr-4">
                                <?php echo htmlspecialchars($item['nama_produk']); ?>
                            </h4>
                            <p class="text-brand-black font-black">Rp
                                <?php echo number_format($item['subtotal'], 0, ',', '.'); ?>
                            </p>
                        </div>
                        <div class="flex justify-between items-center mt-2">
                            <p class="text-sm text-brand-gray font-medium">Rp
                                <?php echo number_format($item['harga_item'], 0, ',', '.'); ?> / item
                            </p>
                            <div class="flex items-center gap-3 bg-brand-offwhite rounded-lg px-2 py-1 border border-black/5">
                                <span
                                    class="text-sm font-bold w-6 text-center text-brand-red"><?php echo $item['jumlah']; ?>x</span>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <div class="p-6 bg-white border-t border-black/5 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
            <div class="flex justify-between items-center mb-6 text-lg">
                ...
                <span class="text-3xl font-black text-brand-red tracking-tight">Rp
                    <?php echo number_format($totalCart, 0, ',', '.'); ?></span>
            </div>

            <form method="POST" action="" class="flex flex-col gap-3">
                <input type="text" name="nama_customer" placeholder="Nama Pelanggan (Opsional)"
                    class="w-full px-4 py-3 bg-brand-offwhite border border-black/5 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-red focus:bg-white transition-colors font-medium">

                <div class="flex gap-2 mt-2">
                    <button type="submit" name="clear_cart"
                        class="px-5 py-4 bg-brand-offwhite text-brand-black font-bold rounded-xl hover:bg-gray-200 transition-colors border border-black/5">
                        <i class="fa-solid fa-trash text-brand-gray"></i>
                    </button>
                    <button type="submit" name="checkout" <?php if (empty($cart))
                        echo 'disabled'; ?>
                        class="flex-1 bg-brand-red hover:bg-brand-darkred disabled:bg-brand-gray/30 disabled:cursor-not-allowed text-white py-4 rounded-xl font-bold text-lg shadow-[0_4px_14px_rgba(230,57,70,0.4)] transition-all flex justify-center items-center gap-2">
